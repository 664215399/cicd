 ...
 * @param {number} [bytes=8] - 要读取的字节数，默认为8字节（足够识别大多数文件类型）
 ...
 * 
 * @param {Uint8Array} fileHeader - 文件头的字节数组（通过readFileHeader获取）
 * @param {number[]} magicBytes - 预期的Magic Bytes数组，例如PNG为[0x89, 0x50, 0x4e, 0x47, ...]
 * @returns {boolean} 如果文件头匹配返回true，否则返回false
 */
function matchMagicBytes(fileHeader, magicBytes) {
 ...
 * @param {string[]} [allowedTypes=['png', 'jpg']] - 允许的文件类型数组，例如 ['png', 'jpg']，默认为 ['png', 'jpg']
 ...
 * ```
 */
export async function validateFileType(file, allowedTypes = ['png', 'jpg']) {
  try {
    // 1. 检查文件扩展名
